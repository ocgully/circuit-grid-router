...
import { createWfcGrid, collapseCell, getWfcCell, propagate, TILE_CONNECTIONS, DIR_OFFSET, ALL_DIRS, ROUTING_TILES, wfcToGrid2D, } from './wfc-core.js';
...
function seedConnection(wfc, col, row, side, edgeId) {
    const outDir = connOutDir(side);
    // Connection must connect outward from the node
    const validTiles = ROUTING_TILES.filter(t => t !== 'empty' && TILE_CONNECTIONS[t].has(outDir));
 ...
 */
function buildAttractionField(cols, rows, connections, edges) {
    const field = new Map();
    ...
    for (const cp of connections) {
        if (!connByEdge.has(cp.edgeId))
            connByEdge.set(cp.edgeId, []);
 ...
 */
function solveWithAttraction(wfc, attraction, maxIterations = 100000) {
 ...
 */
function assignEdgeIds(wfc, connections, edges) {
    const connByEdge = new Map();
    for (const e of edges)
    ...
    for (const [edgeId, ep] of connByEdge) {
        ...
        const queue = [[ep.src.col, ep.src.row]];
        ...
        while (queue.length > 0) {
            ...
            if (!tileConns || tileConns.size === 0)
                continue;
            cell.edgeId = edgeId;
            if (key === targetKey)
                break;
            for (const dir of ALL_DIRS) {
                if (!tileConns.has(dir))
                    continue;
...
// Public API
// ---------------------------------------------------------------------------
export function buildScenarioWfcFull(nodes, edges, padding = 2) {
    let maxCol = 0;
    let maxRow = 0;
    for (const n of nodes) {
...
//# sourceMappingURL=wfc-full.js.map