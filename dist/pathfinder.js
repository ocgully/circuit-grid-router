 ...
 */
function cellBlocked(gx, gy, nodeRects, srcRect, tgtRect, gridSize, nodeMargin) {
...
function lShapeFallback(sx, sy, tx, ty, occupiedSegments, gridSize) {
    ...
    for (const seg of segments)
        markOccupied(seg, occupiedSegments);
    return { waypoints, segments };
}
/** Secondary routing function for coarser grid — same algorithm, different cell size. */
function routeEdgeAtGrid(sx, sy, tx, ty, _nodeRects, occupiedSegments, gridSize) {
 ...
 */
export function routeEdge(sourceX, sourceY, targetX, targetY, nodeRects, occupiedSegments, options, sourceRect, targetRect) {
    ...
    const cellIndex = (gx, gy, dir) => {
        const dirIdx = dir === null ? 0 : dir === 'h' ? 1 : 2;
        return ((gy - minY) * cols + (gx - minX)) * 3 + dirIdx;
    };
    // Heuristic: Manhattan distance
    const h = (gx, gy) => Math.abs(gx - tgtGrid.gx) + Math.abs(gy - tgtGrid.gy);
    ...
    const startNode = { gx: srcGrid.gx, gy: srcGrid.gy, g: 0, f: h(srcGrid.gx, srcGrid.gy), parent: null, dir: null };
    open.push(startNode);
    ...
    while (open.length > 0) {
        ...
        for (const { dx, dy } of DIRS) {
            ...
            if (!isStartOrEnd && cellBlocked(nx, ny, nodeRects, srcRectRef, tgtRectRef, gridSize, opts.nodeMargin))
                continue;
