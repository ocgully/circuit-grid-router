...
export function segmentUnitKeys(seg) {
    const keys = [];
    if (seg.axis === 'h') {
...
export function buildSegments(gridPath) {
    ...
    for (let i = 1; i < gridPath.length; i++) {
        ...
        const axis = prev.gx !== curr.gx ? 'h' : 'v';
