 ...
 */
import { buildScenario as buildScenarioAStar, setCell } from './grid2d.js';
import { createWfcGrid, collapseCell, propagate, solve, } from './wfc-core.js';
/**
 ...
 */
function detectSpacingViolations(grid) {
    const conflicts = [];
    for (let r = 0; r < grid.rows; r++) {
        for (let c = 0; c < grid.cols; c++) {
            const cell = grid.cells[r][c];
...
// ---------------------------------------------------------------------------
function expandZone(conflicts, grid, margin = 2) {
    const zone = new Set();
 ...
 */
function gridCellToWfcTile(cell) {
    switch (cell.type) {
 ...
 */
function rerouteZone(grid, zone, conflictEdgeIds) {
...
// Public API
// ---------------------------------------------------------------------------
export function buildScenarioWfcConflict(nodes, edges, padding = 2) {
...
//# sourceMappingURL=wfc-conflict.js.map