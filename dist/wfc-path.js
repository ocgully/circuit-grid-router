 ...
 */
function growPath(wfc, sc, sr, sourceSide, tc, tr, targetSide, edgeId) {
    ...
    while (queue.length > 0 && iterations < maxIter) {
        ...
        if (cell.collapsed !== null && cell.collapsed !== 'empty')
        ...
        const routingOptions = [...cell.options].filter(t => t !== 'empty' && t !== 'node' && t !== 'blocked' && t !== 'conn');
        ...
        const preferDir = preferH ? (dx > 0 ? 'right' : 'left') : (dy > 0 ? 'down' : 'up');
        // Score tiles: prefer ones connecting toward target
        let bestTile = null;
        let bestScore = -1;
        for (const t of routingOptions) {
            let score = 0;
            if (TILE_CONNECTIONS[t].has(preferDir))
                score += 2;
        ...
        for (const dir of ALL_DIRS) {
            if (!TILE_CONNECTIONS[bestTile].has(dir))
                continue;
...
// Public API
// ---------------------------------------------------------------------------
export function buildScenarioWfcPath(nodes, edges, padding = 2) {
    let maxCol = 0;
    let maxRow = 0;
    for (const n of nodes) {
...
//# sourceMappingURL=wfc-path.js.map