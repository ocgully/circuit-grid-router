...
import type { NodeRect, HandlePosition, Side, RouterOptions } from './types.js';
import { resolveOptions } from './types.js';
import { snapToGrid } from './grid.js';

/** Return the side of `rect` that faces `otherRect` most directly. */
function facingSide(rect: NodeRect, otherRect: NodeRect): Side {
...
function sideLength(rect: NodeRect, side: Side): number {
 ...
 */
export function distributeOnSide(count: number, sideCenter: number, gridSize: number): number[] {
  if (count === 0) return [];
...
export function computeHandlePositions(
  nodeId: string,
  nodeRect: NodeRect,
  connectedEdges: { edgeId: string; direction: 'source' | 'target'; otherNodeRect: NodeRect }[],
  ...
  const sideGroups = new Map<Side, typeof connectedEdges>();
...
// ---------------------------------------------------------------------------
// Batch endpoint computation with symmetric distribution
// ---------------------------------------------------------------------------
 ...
 */
export function computeAllEndpoints(
  edges: EdgeSpec[],
  ...
  // Track: for each (nodeId, side), which edges connect there and sort info
  interface SideEntry {
    edgeId: string;
    role: 'source' | 'target';
    /** Perpendicular coordinate of the OTHER node's center (for sorting). */
    sortKey: number;
  }
  ...
  for (const [groupKey, entries] of sideGroups) {
    ...
    const side = groupKey.slice(sepIdx + 1) as Side;
